...
Sinkronisasi konten picdiary.html ke struktur Activity-Log harian Aasiyah.
...
def _is_abang_only(text: str) -> bool:
    """True jika baris menyebut Abang/Musa tapi tidak menyebut Kakak/Aasiyah."""
    if not text.strip():
        return False
    return bool(_ABANG_RE.search(text)) and not bool(_KAKAK_RE.search(text))


def aasiyah_focus(text: str) -> str:
    """Saring teks agar fokus ke Aasiyah (Kakak). Baris yang hanya tentang Abang/Musa dibuang.
...
def aasiyah_focus_subtitle(text: str) -> str:
    """Untuk subtitle headline: jika hanya tentang Abang/Musa, kosongkan."""
    if not text:
        return text
    if _is_abang_only(text):
        return ""
    return text
...
def main():
    ...
    # Fokuskan narasi ke Aasiyah (Kakak). Baris yang hanya tentang Abang/Musa dihapus,
    # sedangkan baris bersama (mis. "Abang dan Kakak ...") dipertahankan.
    for entry in entries:
        entry.kegiatan = aasiyah_focus(entry.kegiatan)
        entry.capaian = aasiyah_focus(entry.capaian)
        entry.kendala = aasiyah_focus(entry.kendala)
        entry.subtitle = aasiyah_focus_subtitle(entry.subtitle)
